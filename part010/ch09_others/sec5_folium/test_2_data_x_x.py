...
map_a = folium.Map(location=[46.3014, -123.7390], zoom_start=7,tiles='Stamen Terrain')
...
folium.RegularPolygonMarker([47.3489, -124.708],
   fill_color='#ff0000', radius=12, popup=popup1).add_to(map_a)
################################################################################
...
folium.RegularPolygonMarker([44.639, -124.5339],
   fill_color='#00ff00', radius=12, popup=popup2).add_to(map_a)
################################################################################
...
folium.RegularPolygonMarker([46.216, -124.1280],
   fill_color='#0000ff', radius=12, popup=popup3).add_to(map_a)
...
################################################################################
ice_shelf = '/gdata/folium/data/antarctic_ice_shelf_topo.json'
folium.TopoJson(open(ice_shelf),
...
map_c.choropleth(geo_data=state_geo, name='choropleth', data=state_data,
...
map_d.choropleth(geo_data=state_geo, data=state_data,columns=['State', 'Unemployment'],
